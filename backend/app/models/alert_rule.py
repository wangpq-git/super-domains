...
class AlertRule(Base):
    ...
    channels: Mapped[dict] = mapped_column(default=list)
    recipients: Mapped[dict] = mapped_column(default=list)
    apply_to_all: Mapped[bool] = mapped_column(Boolean, default=True)
    specific_platforms: Mapped[dict] = mapped_column(nullable=True)
    specific_domains: Mapped[dict] = mapped_column(nullable=True)
